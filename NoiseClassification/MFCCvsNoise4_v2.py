...
def cross_validate(clf, X, y, features):
    group_kfold = GroupShuffleSplit(n_splits=3, test_size=0.2, random_state=5)
...
if __name__ == '__main__':
    ...
    clf = MLPClassifier(hidden_layer_sizes=(40, 30, 30, 30, 3), activation='relu', solver='adam')
    ...
    start_time = time.time()
    clf = MLPClassifier(hidden_layer_sizes=(40, 30, 30, 30, 3), activation='relu', solver='lbfgs')
    accuracy = test_classifier(type(clf).__name__, clf, X_train, y_train, X_test, y_test)
    training_time = time.time() - start_time
    accuracy_cv = cross_validate(clf, X, y, features)
    ...
    start_time = time.time()
    clf = MLPClassifier(hidden_layer_sizes=(40, 35, 35, 35, 3), activation='relu', solver='adam')
    accuracy = test_classifier(type(clf).__name__, clf, X_train, y_train, X_test, y_test)
    training_time = time.time() - start_time
    accuracy_cv = cross_validate(clf, X, y, features)
    results.loc[len(results)] = [no_mfcc, type(clf).__name__+'NN5adamPlus', accuracy, np.mean(accuracy_cv), accuracy_cv, training_time]

    results.to_csv('./results_4_v2/'+results_file, sep=';', float_format='%.4f')
...
def cross_validate(clf, X, y, features):
    group_kfold = GroupShuffleSplit(n_splits=5, test_size=0.2, random_state=5)
...
if __name__ == '__main__':
    ...
    base_estimator = RandomForestClassifier(n_estimators=10, random_state=1, n_jobs=-1)
    clf = AdaBoostClassifier(base_estimator=base_estimator, n_estimators=50, random_state=0)
    ...
    results.loc[len(results)] = [no_mfcc, type(clf).__name__+'_RFC10', accuracy, np.mean(accuracy_cv), accuracy_cv,
                                 training_time]

    start_time = time.time()
    base_estimator = RandomForestClassifier(n_estimators=20, random_state=1, n_jobs=-1)
    clf = AdaBoostClassifier(base_estimator=base_estimator, n_estimators=50, random_state=0)
    accuracy = test_classifier(type(clf).__name__, clf, X_train, y_train, X_test, y_test)
    training_time = time.time() - start_time
    accuracy_cv = cross_validate(clf, X, y, features)
    results.loc[len(results)] = [no_mfcc, type(clf).__name__+'_RFC20', accuracy, np.mean(accuracy_cv), accuracy_cv, training_time]

    start_time = time.time()
    base_estimator = ExtraTreesClassifier(n_estimators=20, random_state=1, n_jobs=-1)
    clf = AdaBoostClassifier(base_estimator=base_estimator, n_estimators=50, random_state=0)
    accuracy = test_classifier(type(clf).__name__, clf, X_train, y_train, X_test, y_test)
    training_time = time.time() - start_time
    accuracy_cv = cross_validate(clf, X, y, features)
    results.loc[len(results)] = [no_mfcc, type(clf).__name__+'_ETC20', accuracy, np.mean(accuracy_cv), accuracy_cv, training_time]

    # start_time = time.time()
    # clf = AdaBoostClassifier(base_estimator=linear_model.SGDClassifier(n_jobs=-1), algorithm='SAMME', n_estimators=50, random_state=0)
    # accuracy = test_classifier(type(clf).__name__, clf, X_train, y_train, X_test, y_test)
    # training_time = time.time() - start_time
    # accuracy_cv = cross_validate(clf, X, y, features)
    # results.loc[len(results)] = [no_mfcc, type(clf).__name__+'_SGDC', accuracy, np.mean(accuracy_cv), accuracy_cv, training_time]

    results.to_csv('./results_2/'+results_file, sep=';', float_format='%.4f')
